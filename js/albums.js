...
function displayFavoriteAlbums() {
    ...
    textDiv = `
    <button onclick="showCookieBash64()">导出收藏专辑列表</button>
    <button onclick="importFavorites()">导入收藏专辑列表</button>
                    ...
                    <div class="col-lg-2 col-md-3 col-sm-6 col-12 mb-4">
                        <div class="card shadow-sm">
                            <div class="square-img-container">
                                <img class="card-img-top square-img" title="点击打开专辑：${album.name}" src="/cover/${album.coverArt}" alt="${album.name} Album Cover - TouHou Music" onclick="loadAlbumDetails('${album.id}')">
                            </div>
                            <div class="card-body">
    ...
    albums.forEach(function (album) {
        albumsHtml += `
            <div class="col-lg-2 col-md-3 col-sm-6 col-12 mb-4">
                <div class="card shadow-sm">
                    <div class="square-img-container">
                        <img class="card-img-top square-img" title="点击打开专辑：${album.album}" src="/cover/${album.coverArt}" alt="${album.album} Album Cover - TouHou Music" onclick="loadAlbumDetails('${album.id}')">
                    </div>
                    <div class="card-body">
...
<ul class="list-group mb-3">
    ...
    <li class="list-group-item play-album" onclick="playAlbum('${album.id}')"><strong>播放专辑:</strong> ▶总长${formatTime(album.duration)}</li>
    <li class="list-group-item"><button id="saveFavoriteBtn" data-album-id="${album.id}">添加到收藏</button></li>
</ul>
        </div>
    </div>
...
`;
    album.song.forEach(function (song) {
        detailsHtml += `
        <div class="song-item" style="cursor: pointer;" onclick="playSong('${song.id}', '${song.title}', '${album.coverArt}', '${song.artist}')">
